...
for sen in file[:, 1]:
	clean = re.sub("[^a-zA-Z]"," ",sen)
	corp.append(nltk.word_tokenize(clean))

print("Tokens done: {}".format(datetime.now()-start))

count_word = sum(len(x) for x in corp)
print("sum of word frequency in corpora: {}".format(count_word))
#np.savetxt('tokens.txt', corp, fmt='%s', comments=chr(0), delimiter='\t')


#print("Reading tokens")
#corp = np.loadtxt('tokens.txt', dtype=str, delimiter='\t', comments=chr(0))
#print(corp)

print("Skip gram model initialised")

epoch_logger = EpochLogger("./model_sg/nepal")
model = gensim.models.Word2Vec(hs=1,sample=1e-5,size=300,seed=0,sg=1,workers = multiprocessing.cpu_count(), window=7,  callbacks=[epoch_logger])
model.build_vocab(corp)

print("total_sentences: {}, total_words: {}".format(model.corpus_count, len(model.wv.vocab.keys())))
print("Start training...")
model.train(corp, total_examples=model.corpus_count, epochs=300)

print("Sum of word frequency in vocab: {}".format(sum(x.count for x in model.wv.vocab.values())))

print("Final model saved as {}".format('./model_sg/nepal.model'))
model.save('./model_sg/nepal.model')
